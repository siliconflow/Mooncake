...
#
# Builds only to the `wheel-export` stage of docker/mooncake.Dockerfile and
# streams the produced wheel straight to OUTPUT_DIR via BuildKit `--output`
# (no container is run), then uploads it to OSS under a per-build date dir.
#
# The wheel version is stamped with the commit id + CUDA version by the
# Dockerfile (MOONCAKE_WHEEL_VERSION_SUFFIX), so each build is identifiable
# by commit + CUDA variant.
#
# Usage:
...
OSSUTIL="${OSSUTIL:-$HOME/ossutil64}"
PIP_INDEX_URL="${PIP_INDEX_URL:-https://pypi.org/simple}"

# Fixed OSS destination prefix; only the final date segment is dynamic.
OSS_PREFIX="oss://siliconllm/llm-static/whl/mooncake"
...
# ---- 1. Build the wheel-export stage and stream the wheel to OUTPUT_DIR ------
# Uses BuildKit `--output type=local` to extract the wheel built in the builder
# stage directly to the host without running a container. This is required
# under rootless nerdctl, where `docker cp` from a stopped container (created
# via `docker create`) fails with "cannot use cp with stopped containers in
# rootless mode". A tagged builder image is no longer produced.
rm -rf "${OUTPUT_DIR}"
mkdir -p "${OUTPUT_DIR}"
docker build \
    --target wheel-export \
